function [collage, M, N] = hybrid_collage3_rev(img1, img2, img3)


[collage, m, n] = hybrid_collage2(img1, img2);


img3 = imresize(img3, [size(collage, 1), size(collage, 2)]);

M = int16(size(img3, 1) * 0.2);
N = size(img3, 2);

temp = img3(size(img1, 1)-M+1:size(img1, 1), 1:N, :);
...
im1 = collage(1:M, size(collage, 2)/2-N/2+1:size(collage, 2)/2+N/2, :);
im2 = img3(size(img1, 1)-M+1:size(img1, 1), 1:N, :);
...
for i = 1:size(hybrid_image, 2)
    for j = 1:size(hybrid_image, 1)
        ...
        hybrid_image(j, i, :) = (1-alpha)*hybrid_image(j, i, :) + alpha*im1(j, i, :);
    end
    
    for j = 1:size(hybrid_image, 1) % floor(size(hybrid_image, 2)/2):size(hybrid_image, 2)
        alpha = j/double(size(hybrid_image, 1));
        hybrid_image(j, i, :) = (alpha)*hybrid_image(j, i, :) + (1-alpha)*im2(j, i, :);
    end
...
hybrid(size(collage, 1)-M+1:size(hybrid, 1), 1:size(collage, 2), :) = collage;


hybrid(1:size(img3, 1), size(hybrid, 2)/2-N/2+1:size(hybrid, 2)/2+N/2, :) = img3;
